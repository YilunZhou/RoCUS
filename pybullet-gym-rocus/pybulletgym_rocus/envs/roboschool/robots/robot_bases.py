...
class XmlBasedRobot:
	...
	def __init__(self, robot_name, action_dim, obs_dim, self_collision):
		...
		high = np.ones([action_dim]).astype('float32')
		self.action_space = gym.spaces.Box(-high, high)
		high = np.inf * np.ones([obs_dim]).astype('float32')
	...
	def reset(self, bullet_client, **kwargs):
		...
		if self.doneLoading == 0:
			...
			if self.self_collision:
				...
				self.parts, self.jdict, self.ordered_joints, self.robot_body = self.addToScene(self._p, self.objects)
		self.robot_specific_reset(self._p, **kwargs)
...
class URDFBasedRobot(XmlBasedRobot):
	...
	def __init__(self, model_urdf, robot_name, action_dim, obs_dim, basePosition=[0, 0, 0], baseOrientation=[0, 0, 0, 1], 
				 fixed_base=False, self_collision=False, additional_urdfs=[], collision_flags=None):
		XmlBasedRobot.__init__(self, robot_name, action_dim, obs_dim, self_collision)
	...
	def set_joint_positions(self, body, joints, values):
		...
		for joint, value in zip(joints, values):
			self._p.resetJointState(body, joint.jointIndex, value)
		...
		if not self.doneLoading:
			...
			if self.self_collision:
				if self.collision_flags is None:
					self.collision_flags = pybullet.URDF_USE_SELF_COLLISION | pybullet.URDF_USE_SELF_COLLISION_EXCLUDE_ALL_PARENTS
				...
				self.parts, self.jdict, self.ordered_joints, self.robot_body = self.addToScene(self._p,
					...
					baseOrientation=self.baseOrientation,
					useFixedBase=self.fixed_base))
			for urdf, urdf_kwargs in self.additional_urdfs:
				print('Loading', urdf)
				self._p.loadURDF(urdf, **urdf_kwargs)
		self.robot_specific_reset(self._p, **kwargs)
...
class SDFBasedRobot(XmlBasedRobot):
	...
	def __init__(self, model_sdf, robot_name, action_dim, obs_dim, basePosition=[0, 0, 0], baseOrientation=[0, 0, 0, 1], fixed_base=False, self_collision=False):
		XmlBasedRobot.__init__(self, robot_name, action_dim, obs_dim, self_collision)
		raise Exception('done loading not implemented')
